...
# sklearn에서 관련 모듈을 임포트
from sklearn.metrics import roc_curve, auc

# ROC 곡선을 그리기 위한 값과 AUC 값을 변수에 저장
fpr_A, tpr_A, thrA = roc_curve(y_true, A_proba)
fpr_B, tpr_B, thrB = roc_curve(y_true, B_proba)
fpr_C, tpr_C, thrC = roc_curve(y_true, C_proba)
...
plt.legend(loc="lower right")   # 범례를 추가
plt.tight_layout()
plt.show()
